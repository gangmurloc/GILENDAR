...
LLM API(gpt-5.6-luna, OpenAI 호환 Chat Completions)가 이를 파싱해서 등록 전 미리보기를 보여주고, 확인하면 구글 캘린더에 자동으로 등록해주는 개인용 봇입니다.
...
### 1-3. LLM API 키 발급
1. 사용 중인 게이트웨이(예: 학교/기관에서 제공하는 OpenAI 호환 API)에서 API 키 발급.
2. `.env`의 `FACTCHAT_API_KEY`, `FACTCHAT_BASE_URL`, `FACTCHAT_MODEL`에 저장.
   - 이미지(사진 시간표)는 지원하지만 음성 메시지는 지원하지 않는 모델이라, 이 봇도 음성 메시지는 처리하지 않습니다.
...
텔레그램에서 봇에게 자연어로 말하면 됩니다 — 텍스트, 사진(시간표/포스터) 모두 가능합니다.
...
(잘못 해석했으면 "❌ 취소"로 무시하면 됩니다). 일정을 삭제하면 "↩️ 되돌리기" 버튼으로 바로 복구할 수 있습니다.

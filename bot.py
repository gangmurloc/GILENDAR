...
from llm_parser import handle_message
...
def _friendly_error_message(exc: Exception, fallback: str) -> str:
    if isinstance(exc, openai.RateLimitError):
        return "⏳ 지금 요청이 몰려서 처리할 수 없어요. 잠시 후 다시 시도해 주세요."
    return fallback
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "시간표나 포스터 사진을 보내도 읽어서 일정으로 등록할 수 있어요.\n\n"
...
async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        logger.exception("LLM image request failed")
...
async def handle_voice(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not _is_owner(update):
        await update.message.reply_text("이 봇은 개인용입니다.")
        return
    await update.message.reply_text("죄송해요, 지금은 음성 메시지를 처리할 수 없어요. 텍스트나 사진으로 보내주세요.")
